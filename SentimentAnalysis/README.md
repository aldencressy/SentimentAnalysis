...
1. How do you classify a new tweet? Give a short description of the main steps.
    ...
    > First, I created a dictionary with all the words from the training dataset. Each word in the dictionary has a positive and negative counter that is incremented each time that word appears in a positive/negative tweet. Then, I compare each word in the testing dataset to the words in the dictionary. Depending on how many positive/negative occurrances the word has, it will increment the positive or negative counter for the tweet. Once all the words in the tweet have been compared to the words in the dictionary, the tweet is classified based off of whether the positive or the negative counter for the tweet is higher.

2. What is the time complexity of your training implementation (Big-Oh notation)? Explain why.

   > The time complexity of my training implementation is o(n)^3 "o n cubed". This is because it uses a triple for loop when creating the dictionary.

3. What is the time complexity of your classification implementation (Big-Oh notation)? Explain why.

   > The time complexity of my classification implementation is also o(n)^3 "o n cubed". This is because it uses a triple for loop to classify the Tweets.

4. How do you know that you use proper memory management? That is that you do not have
   a memory leak?

   > The only thing that I declare on the heap are the character arrays in my DSString class. The destructor takes care of these. In my model and train functions, I declare a cstring on the heap. At the end of both functions, I manually delete these cstrings. DSVector was provided to us by the teacher, and it has proper memory managment through the use of the destructor.

5. What was the most challenging part of the assignment?

   > The most challenging part of this assignment was creating the dictionary. I was getting horrible memory errors that I spent hours trying to debug.
